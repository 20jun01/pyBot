...
async def root(request: Request):
    if verification_handler(request.headers):
...
async def generate_talk(request: TalkRequest):
    ...
    new_message, past_messages = talk_handler.talk_handler([], request.message, system_settings)
...
async def generate_talk(request: TalkRequest):
    global past_messages
    new_message, messages = talk_handler.talk_handler(past_messages, request.message, system_settings)
    return TalkResponse(message=":@BOT_urturn225: < " + new_message)
